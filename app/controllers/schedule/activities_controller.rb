class Schedule::ActivitiesController < ApplicationController
  before_action :set_schedule_activity, only: [:show, :edit, :update, :destroy]

  respond_to :html

  def index
    @schedule_activities = Schedule::Activity.all
  ...
  def new
    @schedule_activity = Schedule::Activity.new
    respond_with(@schedule_activity)
  end
  ...
  def create
    @schedule_activity = Schedule::Activity.new(activity_params)
  ...
  private
    def set_schedule_activity
      @schedule_activity = Schedule::Activity.find(params[:id])
    end
